...
function title() {
    console.log(chalk.bold.green(figlet.textSync('IKYBOT', {
        font: 'Standard',
...
function uncache(module = '.') {
    ...
    const haruka = harukaConnect({
      version,
        ...
        browser: ['Iky Multi Device','Safari','1.0.0'],
        auth: state
    })
...
app.use(express.static(path.join(__dirname, 'views')))
  server.listen(PORT, () => {
    ...
    let nomer = await haruka.sendTextWithMentions(kopel.from, `*${haruka.user.name}* 𝑻𝒊𝒅𝒂𝒌 𝑫𝒂𝒑𝒂𝒕 𝑴𝒆𝒏𝒆𝒓𝒊𝒎𝒂 𝑷𝒂𝒏𝒈𝒈𝒊𝒍𝒂𝒏 ${kopel.isVideo ? `video` : `suara`}. 𝑴𝒂𝒂𝒇 @${kopel.from.split('@')[0]}  𝑲𝒂𝒎𝒖 𝑨𝒌𝒂𝒏 𝑫𝒊 𝑩𝒍𝒐𝒌𝒊𝒓, 𝑺𝒊𝒍𝒂𝒉𝒌𝒂𝒏 𝑯𝒖𝒃𝒖𝒏𝒈𝒊 𝑶𝒘𝒏𝒆𝒓 𝑼𝒏𝒕𝒖𝒌 𝑴𝒆𝒎𝒃𝒖𝒌𝒂 𝑩𝒍𝒐𝒌𝒊𝒓`)
